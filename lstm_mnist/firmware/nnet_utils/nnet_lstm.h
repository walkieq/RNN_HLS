...
// pass with small lx and lh, full pipeline without resource reuse.
template<class data_T, class res_T, typename CONFIG_T, typename CONFIG_A>
void lstm_01(
    ...
    //const int multiplier_limit = CONFIG_T::length_h * 4 * 4; //compute_multiplier_limit_conv2d<CONFIG_T>(weights);
    //#pragma HLS ALLOCATION instances=mul limit=multiplier_limit operation
    ...
    TIMESTEP:for(int its = 0; its < CONFIG_T::timestep; its++) {
    	...
    	for(int ix = 0; ix < CONFIG_T::length_x; ix++){
    		for(int jx = 0; jx < CONFIG_T::length_h*4; jx++){
    			mult_x[ix*CONFIG_T::length_h*4+jx] = data[ix+its*CONFIG_T::length_x] * weights_x[ix*CONFIG_T::length_h*4+jx];
    		}
    	}
    	for(int ih = 0; ih < CONFIG_T::length_h; ih++){
    		for(int jh = 0; jh < CONFIG_T::length_h*4; jh++){
    ...
    TEST:for(int ii = 0; ii < CONFIG_T::length_h; ii++) {
		#pragma HLS unroll
